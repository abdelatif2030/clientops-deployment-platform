        ...
        stage('Generate Inventory') {
            steps {
                sh '''
                    ...
                    # Generate inventory without extra "ok"
        ...
        stage('Wait for SSH Access') {
            steps {
                sh '''
                    ...
                    for i in {1..12}; do
                        ssh -i $SSH_KEY -o StrictHostKeyChecking=no -o ConnectTimeout=5 $APP_USER@$APP_IP 'exit' && \
                        ssh -i $SSH_KEY -o StrictHostKeyChecking=no -o ConnectTimeout=5 $MONITOR_USER@$MONITOR_IP 'exit' && break
